...
/* THREEJS WORLD SETUP */
let currentVrm;
// Add these imports at the top of your main.js

// Voice Chat Configuration
let isListening = false;
let isSpeaking = false;
let chatActive = false; // Controls the conversation state
let silenceTimeout;
let responseTimeout;
...
function startVoiceChat() {
    ...
    // Stop recognition if no speech is detected within 5 seconds
...
recognition.onresult = async (event) => {
    ...
    try {
        ...
        responseTimeout = setTimeout(() => {
            updateStatus("No response, moving to next question...");
            stopChat(); // Stop chat if no response within 15 seconds
        }, 15000);

        // Get AI response
        const result = await model.generateContent({ contents: [{ role: "user", parts: [{ text: transcript }] }] });
...
function speakChunks(chunks) {
    ...
    let utterance = new SpeechSynthesisUtterance(chunks.shift());
    utterance.rate = 1.0;
    utterance.pitch = 1.0;
